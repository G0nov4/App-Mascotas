...
router.get('/list/:id', async (req, res)=>{
    ...
    const pet = await pool.query('SELECT u.name, u.email, u.phone, p.name pet_name,p.specie, p.size, p.datepet,p.sex,p.longitud, p.latitud, p.observation FROM user u, pet p WHERE p.iduser = u.iduser and p.idpet = ?',[req.params.id]);
    console.log(pet[0])
    if(pet[0]!==undefined){
